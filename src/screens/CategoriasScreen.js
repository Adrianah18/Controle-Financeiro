...
const CategoriasScreen = () => {
  ...
  const [novaCategoria, setNovaCategoria] = useState({
    nome: '',
    descricao: '',
    tipo: 'Despesas', // Defina o valor inicial como 'Despesas'
    orcamento: '',
    prioridade: null,
  });
...
{categorias.map((item, index) => (
  <View style={styles.categoryItem} key={index}>
    <Text style={{ color: getPrioridadeColor(item.prioridade) }}>
      <Text style={styles.categoryDetail}>{renderIcon(item.tipo)}</Text>
      {' '}
      <Text style={styles.categoryDetail}>Nome: {item.nome}</Text>
      {'\nDescrição: '}
      <Text style={styles.categoryDetail}>{item.descricao}</Text>
      {'\nTipo: '}
      <Text style={styles.categoryDetail}>{item.tipo}</Text>
      {'\nOrçamento: '}
      <Text style={styles.categoryDetail}>{item.orcamento}</Text>
      {'\nPrioridade: '}
      <Text style={styles.categoryDetail}>{item.prioridade}</Text>
    </Text>
