      ...
      {despesas.map((item, index) => (
        <View key={index} style={styles.despesaItem}>
          <Text style={styles.despesaText}>
            <Text style={styles.despesaDetail}>Nome:</Text> {item.nome}{'\n'}
            <Text style={styles.despesaDetail}>Valor: R$ </Text>
            <Text style={[styles.expenseValue, styles.expenseDetail]}>{item.valor}</Text>
            {'\n'}
            <Text style={styles.despesaDetail}>Vencimento:</Text> {item.dataVencimento}{'\n'}
            <Text style={styles.despesaDetail}>Categoria:</Text> {item.categoria}{'\n'}
            <Text style={styles.despesaDetail}>Recorrência:</Text> {item.recorrencia}
          </Text>
          <View style={styles.botoesAcao}>
            <TouchableOpacity onPress={() => handleEditarDespesa(index)}>
              <View style={styles.editButton}>
                <Text style={styles.textoBotaoAcao}>Editar</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => handleExcluirDespesa(index)}>
              <View style={styles.deleteButton}>
                <Text style={styles.textoBotaoAcao}>Excluir</Text>
              </View>
            </TouchableOpacity>
          </View>
          {index < despesas.length - 1 && <View style={styles.despesaSeparator} />}
        </View>
      ))}
    </ScrollView>
  );
